...
class RollbackParamSource(BaseModel):
    """Declares where a single rollback parameter value comes from.

    Used in the ``rollback_params`` section of action YAML.
    The ``source`` string uses dot-path syntax: ``params.<name>`` copies
    from the original action params, ``before_state.<name>`` restores
...
class ActionDefinition(BaseModel):
    ...
    rollback_params: dict[str, RollbackParamSource] = Field(
...
class RollbackPlan(BaseModel):
    """A structured plan for reversing a previously executed action.

    Generated by ``generate_rollback()``. Does NOT execute anything —
    the agent decides whether to call ``check_rollback()`` and then execute.
    """
